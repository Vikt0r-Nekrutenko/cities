...
ant::ant(matrix &map, edge &start_location)
    : m_global_map(&map)
    , m_personal_map(map)
    , m_location(&m_personal_map[start_location.next_vertex()]
                                [0]) // every time starting from the edge ending "a"
{
...
vertex *ant::step_forward()
{
    ...
    m_passed_path.add_passed_edge(
        (*m_global_map)[current_vrtx][next_vrtx][index]); // store the current edge
    m_location->rm_edge();                                // remove passed edge from map

    if (!m_location->empty()
        && m_location->longest_edge()
               .is_loop()) // if next edge in current vertex is loop, at first go along it
        return &m_personal_map[m_location->longest_edge().current_vertex()]
                              [m_location->longest_edge().next_vertex()];
    else
    ...
    for_each(m_personal_map[next].begin(), m_personal_map[next].end(), [&](vertex &vrtx) {
        if (!vrtx.empty()) {
            ...
            float probability = 100.f
                                * (tao * eta / summary_probability); // total probability for edge
...
void ant::update_pheromone()
{
    float pheromone
        = 0.1f
          - 1.f / m_passed_path.length(); // the larger the path, the more pheromone needs to be left
