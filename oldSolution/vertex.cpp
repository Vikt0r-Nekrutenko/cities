...
{
    for (std::vector<edge>::iterator it = m_outgoing_edges.begin(); it != m_outgoing_edges.end();
         it++)
        it->evaporation();
}
...
void vertex::sort()
{
    for (std::vector<edge>::iterator it = m_outgoing_edges.begin();
         it != m_outgoing_edges.end() - 1;
         it++) {
